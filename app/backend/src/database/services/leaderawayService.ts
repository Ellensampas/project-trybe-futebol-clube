...
const leaderBoardHome = async () => {
  ...
  const obj = (await times).map((tim) => {
    const part = partidasFinalizadas.filter((par) => par.awayTeamId === tim.id);
    const timeList = {
      name: tim.teamName,
      totalPoints: 0,
      totalGames: part.length,
      totalVictories: part.filter((parti) => parti.awayTeamGoals > parti.homeTeamGoals).length,
      totalDraws: part.filter((parti) => parti.homeTeamGoals === parti.awayTeamGoals).length,
      totalLosses: part.filter((parti) => parti.homeTeamGoals > parti.awayTeamGoals).length,
